...
/**
 * Player — patched by rdLoader.
 *
 * Hook added: jump is now routed through RDLoader.firePlayerJump(),
 * allowing mods to cancel or modify jump force.
 */
public class Player {
    private Level level;
    public float xo;
    ...
    public void tick() {
        ...
        if (Keyboard.isKeyDown(Keyboard.KEY_UP)   || Keyboard.isKeyDown(Keyboard.KEY_W)) --ya;
        if (Keyboard.isKeyDown(Keyboard.KEY_DOWN)  || Keyboard.isKeyDown(Keyboard.KEY_S)) ++ya;
        if (Keyboard.isKeyDown(Keyboard.KEY_LEFT)  || Keyboard.isKeyDown(Keyboard.KEY_A)) --xa;
        if (Keyboard.isKeyDown(Keyboard.KEY_RIGHT) || Keyboard.isKeyDown(Keyboard.KEY_D)) ++xa;

        if (Keyboard.isKeyDown(Keyboard.KEY_SPACE) && this.onGround) {
            // ─── rdLoader: jump hook ─────────────────────────────────────
            float jumpForce = RDLoader.firePlayerJump(this, 0.12F);
            if (jumpForce != 0f) {
                this.yd = jumpForce;
            }
        }
