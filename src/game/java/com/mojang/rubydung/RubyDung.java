...
import com.mojang.rubydung.modloader.ModList;
...
/**
 * RubyDung — patched by rdLoader.
 *
 * Changes from original:
 *   - RDLoader.preInit() in static block (mod discovery phase)
 *   - RDLoader.init(this) at end of init()
 *   - RDLoader.postInit() right after
 *   - RDLoader.fireTick(this) in tick()
 *   - RDLoader.fireRender(this, a) in render()
 *   - RDLoader.fireBlockBreak/Place in render() mouse handling
 *   - RDLoader.fireKeyPress in keyboard handling
 *   - RDLoader.fireMouseClick in mouse handling
 *   - RDLoader.firePick after pick()
 */
public class RubyDung implements Runnable {
    ...
    // ─── rdLoader: Phase 0 — mods register themselves ───────────────────
    // Mod classes are referenced here so TeaVM includes them in the build.
    // Add your mod class's static init block, or add it to ModList.java.
    static {
        // ModList.registerAll() — регистрирует все моды из ModList.java.
        // Добавляй свои моды туда.
        ModList.registerAll();
        // После регистрации запускаем preInit.
        RDLoader.preInit();
    }
    ...
    public void tick() {
        // ─── rdLoader: player tick event ────────────────────────────────
        RDLoader.firePlayerTick(this.player);

        this.player.tick();
        levelSave();

        // ─── rdLoader: game tick event ──────────────────────────────────
        RDLoader.fireTick(this);
    }
    ...
    private void pick(float a) {
        ...
        for (int x = (int) Math.floor(px - reach); x <= (int) Math.floor(px + reach); x++) {
            for (int y = (int) Math.floor(py - (reach + 1)); y <= (int) Math.floor(py + (reach + 1)); y++) {
                for (int z = (int) Math.floor(pz - reach); z <= (int) Math.floor(pz + reach); z++) {
                    ...
                    if (block != 0) {
                        double txmin = (x - px) / dx, txmax = (x + 1 - px) / dx;
                        if (txmin > txmax) { double t = txmin; txmin = txmax; txmax = t; }
                        double tymin = (y - py) / dy, tymax = (y + 1 - py) / dy;
                        if (tymin > tymax) { double t = tymin; tymin = tymax; tymax = t; }
                        double tzmin = (z - pz) / dz, tzmax = (z + 1 - pz) / dz;
                        ...
                        if (tEnter <= tExit && tEnter < closestT && tEnter >= 0 && tEnter <= reach + 1) {
                            closestT = tEnter;
                            int face;
                            if      (tEnter == txmin) face = dx > 0 ? 4 : 5;
                            else if (tEnter == tymin) face = dy > 0 ? 0 : 1;
                            else                      face = dz > 0 ? 2 : 3;
        ...
        // ─── rdLoader: fire pick event ───────────────────────────────────
        RDLoader.firePick(this.hitResult);
    }
    ...
    public void render(float a) {
        ...
        while (Mouse.next()) {
            ...
            // ─── rdLoader: mouse click event ────────────────────────────
            if (Mouse.getEventButtonState() && button >= 0) {
                if (RDLoader.fireMouseClick(button)) {
                    continue; // consumed by a mod
                }
            }

            // Right-click: break block
            if (button == 1 && Mouse.getEventButtonState() && this.hitResult != null) {
                int oldType = this.level.getTile(this.hitResult.x, this.hitResult.y, this.hitResult.z);
                // ─── rdLoader: block break event ────────────────────────
                boolean cancelled = RDLoader.fireBlockBreak(
                        this.level, this.hitResult.x, this.hitResult.y, this.hitResult.z, oldType);
                if (!cancelled) {
            ...
            if (button == 0 && Mouse.getEventButtonState() && this.hitResult != null) {
                ...
                // ─── rdLoader: block place event ────────────────────────
                boolean cancelled = RDLoader.fireBlockPlace(this.level, x, y, z, 1);
                if (!cancelled) {
        ...
        while (Keyboard.next()) {
            ...
            // ─── rdLoader: key press event ──────────────────────────────
            if (Keyboard.getEventKeyState()) {
                if (RDLoader.fireKeyPress(key)) {
                    continue; // consumed by a mod
                }
            }
            ...
            if (key == Keyboard.KEY_BACK && Keyboard.getEventKeyState()) {
                this.level.reset();
        ...
        // ─── rdLoader: render event ──────────────────────────────────────
        RDLoader.fireRender(this, a);

        Display.update();
    }

    // ─── rdLoader: expose game internals to mods ─────────────────────────
