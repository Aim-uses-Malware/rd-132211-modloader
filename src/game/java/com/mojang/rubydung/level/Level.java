...
/**
 * Level — patched by rdLoader.
 *
 * Hooks added:
 *   - fireLevelLoad  after load()
 *   - fireLevelSave  before save() (cancellable)
 *   - fireLevelReset before reset() (cancellable)
 *   - fireTileChanged on every setTile()
 */
public class Level {
    ...
    public void load() {
        ...
        // ─── rdLoader: level load event ──────────────────────────────────
        RDLoader.fireLevelLoad(this);
    }

    public void save() {
        // ─── rdLoader: level save event (cancellable) ────────────────────
        if (RDLoader.fireLevelSave(this)) return;
    ...
    public void reset() {
        // ─── rdLoader: level reset event (cancellable) ───────────────────
        if (RDLoader.fireLevelReset(this)) return;
    ...
    public void removeListener(LevelListener l) { this.levelListeners.remove(l); }
    ...
    public boolean isLightBlocker(int x, int y, int z) { return this.isSolidTile(x, y, z); }
    ...
    public ArrayList<AABB> getCubes(AABB aABB) {
        ...
        int x0 = (int) aABB.x0, x1 = (int)(aABB.x1 + 1.0F);
        int y0 = (int) aABB.y0, y1 = (int)(aABB.y1 + 1.0F);
        int z0 = (int) aABB.z0, z1 = (int)(aABB.z1 + 1.0F);
        if (x0 < 0) x0 = 0;
        if (y0 < 0) y0 = 0;
        if (z0 < 0) z0 = 0;
    ...
    public void setTile(int x, int y, int z, int type) {
        if (x >= 0 && y >= 0 && z >= 0 && x < this.width && y < this.depth && z < this.height) {
            ...
            // ─── rdLoader: tile changed event ────────────────────────────
            RDLoader.fireTileChanged(this, x, y, z, type);
        }
    }

    /** Direct raw block access (bypasses events — for mod use only). */
    public void setTileRaw(int x, int y, int z, int type) {
        if (x >= 0 && y >= 0 && z >= 0 && x < this.width && y < this.depth && z < this.height) {
            this.blocks[(y * this.height + z) * this.width + x] = (byte) type;
            this.calcLightDepths(x, z, 1, 1);
            for (int i = 0; i < this.levelListeners.size(); ++i) {
